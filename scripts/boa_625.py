...
class App:
    ...
    def __init__(self, url = 'https://www.bankofamerica.com/mortgage/mortgage-rates/ ',Pur_pri='625,000', zipcode='10001',
                 down_pay = '125,000'):
...
if __name__ == '__main__':
    ...
    df1 = pd.read_csv(output_path+"BOA_Data_Mortgage_03_06_2018.csv")
    df2 = pd.read_csv(output_path+"BOA_Data_Mortgage_375.csv")
    df3 = pd.read_csv(output_path+"BOA_Data_Mortgage_625.csv")
    df = pd.concat([df1, df2, df3])
    df.to_csv(output_path + "BOA_Data_Mortgage_{}.csv".format(now.strftime("%m_%d_%Y")), index=False)