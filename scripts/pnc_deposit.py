...
if result is not None:
    ...
    if currentRates is not None:
        ...
        df = pd.DataFrame(table[1:],columns=['Balance', 'Product_Interest', 'Product_Apy', 'Bank_Name', 'Date','Bank_Product_Name', 'Bank_Product_Type', 'Product_Term','Bank_Product','Bank_Offer_Feature'])
    ...
    if conSavingsLink is not None:
        ...
        if stdSavings is not None:
            ...
            # print(tabulate(savingsTable))

            stdf = pd.DataFrame(savingsTable[2:],columns=['Balance', 'Product_Interest', 'Product_Apy', 'Bank_Name', 'Date','Bank_Product_Name', 'Bank_Product_Type', 'Product_Term','Bank_Product','Bank_Offer_Feature'])
            stdf = stdf[order]
            # print(stdf)
        else:
        ...
        if sSavings is not None:
                ...
                if sSavings_data is not None:
                    for ssd_s_d in sSavings_data[3:]:
                        ...
                        if ssd_s_d_tds is not None:
                            ...
                            # savingsTableData = [re.sub(r'[ +,\n,\t]', '', ssd_s_d_td.text.strip()) for ssd_s_d_td in ssd_s_d_tds]
            ...
            s_saving_df = pd.DataFrame(savingsTable[2:],columns=['Balance', 'Product_Interest', 'Product_Apy', 'Bank_Name', 'Date','Bank_Product_Name', 'Bank_Product_Type', 'Product_Term','Bank_Product','Bank_Offer_Feature'])
        ...
        # print(tabulate(savingsTable))
        # sf = pd.DataFrame(savingsTable)
        # sf.to_csv('savingsTable.csv', index=False)
    else:
    ...
    if MMA is not None:
        ...
        # print(mf)
        # mf.to_csv('moneyTable.csv', index=False)
    else:
