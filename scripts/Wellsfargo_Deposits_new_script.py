...
path = output_path+"Consolidate_WellsF_Data_Saving_Acc"+today.strftime("%m_%d_%Y")+'.csv'
# path = "Consolidate_WellsF_Data_Saving_Acc"+today.strftime("%m_%d_%Y")+'.csv'
...
# print(str(resp).replace('\\t','').replace('\\n',''))
# sys.exit(0)
...
if h1 is not None:
    ...
    Exceldata.append(g)
order = ['bank_product_name', 'Minimu_Balance','Balance', 'Term', 'Interest Rate','APY']
...
df = pd.DataFrame(Exceldata,columns=tableHeaders)
# df = df[order]
