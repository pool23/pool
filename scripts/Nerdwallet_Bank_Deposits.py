...
for i in range(1,len(pages)+1):
    ...
    for offer in  resp['data']['offers']:
        ...
        if Bank_name.lower() in neededBanks:

            a = [Bank_name, 'CD', product_name, minimum_balance, 'Online' if Bank_name.lower() in online_bank else 'Offline', days,'Interest', str(float(apy))+'%']
    ...
    for offer in  resp['data']['offers']:
        ...
        if Bank_name.lower() in neededBanks:
            if Bank_name.lower() in online_bank:
                online = 'Online'
            a = [Bank_name, 'Checking', product_name, fee, online, 'Term_in_Months', 'Interest', np.nan]
            Excel_Table.append(a)
...
for i in range(1,len(pages)+1):
    ...
    for offer in  data['data']['offers']:
        ...
        if bank_name.lower() in neededBanks:
            ...
            a = [offer['product']['institution']['name'], 'Savings', offer['product']['details']['display_name']['value'], offer['details']['minimum_balance']['value'], "Online" if bank_name.lower() in online_bank else 'Offline', 'Term_in_Months', 'Interest', str(float(offer['details']['apy']['value']))+'%']
            Excel_Table.append(a)
...
print('Total Execution Time:',time.time()-startTime,' Seconds')